...
IoController::IoController()
{
    ...
    for(int i=0; i<ledCount(); i++) {
     ...
     for(int i=0; i<buttonCount(); i++) {
         _lastButtonStates.append(isButtonActive(static_cast<Buttons>(i)));
...
bool IoController::isButtonActive(IoController::Buttons button) const
{
    #ifdef IS_EMBEDDED
...
bool IoController::isLedActive(IoController::Leds led) const
{
    #ifdef IS_EMBEDDED
...
void IoController::timerEvent(QTimerEvent*)
{
    for(int i=0; i<buttonCount(); i++) {
        ...
        bool curState = isButtonActive(b);
